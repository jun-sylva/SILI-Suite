...
export default function DiagnosticPage() {
  ...
  function generateHistory(baseCpu: number, baseRam: number) {
    ...
    let currentCpu = baseCpu
    let currentRam = baseRam
    for (let i = 20; i >= 0; i--) {
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        >
          ...
          {isScanning ? 'Actualisation...' : 'Actualiser Rapport'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity"><Server className="h-32 w-32" /></div>
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 bg-emerald-50 text-emerald-600 rounded-lg flex items-center justify-center shrink-0"><CheckCircle2 className="h-5 w-5" /></div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity"><Database className="h-32 w-32" /></div>
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center shrink-0"><Database className="h-5 w-5" /></div>
            <p className="text-sm font-bold uppercase tracking-wider text-slate-500">PostgreSQL DB</p>
          </div>
          <div>
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-black text-slate-800">{dbLatency !== null ? (dbLatency === -1 ? 'N/A' : dbLatency) : '--'}</h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity"><Cpu className="h-32 w-32" /></div>
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 bg-orange-50 text-orange-600 rounded-lg flex items-center justify-center shrink-0"><Cpu className="h-5 w-5" /></div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity"><HardDrive className="h-32 w-32" /></div>
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center shrink-0"><HardDrive className="h-5 w-5" /></div>
            ...
            {history.length > 0 ? (
              <>
                ...
                <div className="flex justify-center gap-6 mt-4">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-orange-500"></div><span className="text-sm font-medium text-slate-600">Charge CPU</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500"></div><span className="text-sm font-medium text-slate-600">Usage RAM</span></div>
                </div>
              </>
            ) : (
              <div className="h-[300px] flex items-center justify-center text-slate-400">
                <p className="text-sm">Lancez un diagnostic pour afficher le graphique.</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col">
          ...
          <CardContent className="p-0 flex-1">
            <div className="divide-y divide-slate-100 h-full">
              {[
                { icon: Database, label: 'Supabase BDD', desc: 'PostgreSQL (Relational)', status: 'Ligne' },
                { icon: ShieldCheck, label: 'GoTrue Auth', desc: 'JWT & OAuth', status: 'Ligne' },
                { icon: HardDrive, label: 'Stockage S3', desc: 'Supabase Storage', status: 'Ligne' },
                { icon: Zap, label: 'Edge Functions', desc: 'Deno Serverless', status: 'Veille' },
                ...
                <div key={label} className="p-5 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                  ...
                  <span className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider ${status === 'Ligne' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>{status}</span>
                </div>
              ))}
            </div>
          </CardContent>
