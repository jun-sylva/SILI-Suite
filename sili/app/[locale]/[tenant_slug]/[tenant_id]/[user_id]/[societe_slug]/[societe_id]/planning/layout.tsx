...
import { LayoutDashboard, FolderKanban, CalendarDays, Users, Loader2, ShieldOff } from 'lucide-react'
...
function PlanningNavItem({ item, isActive }: { item: NavItem; isActive: boolean }) {
...
export default function PlanningLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function check() {
      ...
      const LEVELS = ['lecteur', 'contributeur', 'gestionnaire', 'admin']
      setCanAccess(LEVELS.includes(perm))
      setChecking(false)
    }
    check()
  }, [societeId, router])
  ...
  const navItems: NavItem[] = [
    ...
    { id: 'projets',    label: t('nav_projets'),     href: `${base}/projets`,       icon: FolderKanban,    exact: false },
    { id: 'calendrier', label: t('nav_calendrier'),  href: `${base}/calendrier`,    icon: CalendarDays,    exact: false },
    { id: 'ressources', label: t('nav_ressources'),  href: `${base}/ressources`,    icon: Users,           exact: false },
  ]
        ...
        <nav className="flex gap-1 overflow-x-auto scrollbar-hide">
          {navItems.map((item) => (
            <PlanningNavItem
              key={item.id}
              item={item}
              isActive={item.exact ? pathname === item.href || pathname.endsWith('/planning') : pathname.startsWith(item.href)}
            />
          ))}
        </nav>
      </div>
